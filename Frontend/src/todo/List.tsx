...
const List = () => {
  const { loading, data, mutate } = useFetch();
  ...
  //update todo
  const update = (method?, id?, url?, updata?, preview?) => {
    setShow(!show);
    handleCrud(method, id, url, updata, preview);
    setTimeout(mutate, 200);
  };

  return (
    <>
      <div className="list overflow-x-hidden overflow-y-scroll w-fit px-6 pb-4 mt-4 scrollbar-thin  scrollbar-thumb-[#d4c8b8] scrollbar-track-[#F2EFE6] shadow-inner shadow-[#F2EFE6] rounded-md big:w-[95%] big:m-auto big:mt-4">
        {loading && <p>Loading...</p>}
        {data &&
          data.map((el: any) => (
            <Draggable key={el.id} axis="y" handle={".drag"}>
              <div
                key={el.id}
                className="m-auto relative flex flex-col gap-4 mt-4 glass-component !p-3 big:w-full"
              >
                <button
                  data-bs-toggle="tooltip"
                  data-bs-placement="left"
                  title="Drag"
                  className=" drag absolute bottom-0 right-[-1rem] mr-[1rem] btn-notes btn !rounded-full active:cursor-grabbing !p-[0.11rem] border-none"
                >
                  <Icon
                    path={mdiDragVariant}
                    size={0.7}
                ...
                <div className="flex justify-between gap-4 items-center">
                  <div className="flex flex-row-reverse gap-4">
                    <input
                      type="checkbox"
                      defaultChecked={el.attributes.checkbox}
                  ...
                  <div className="btn-sm flex ">
                    ...
                    <button
                      ...
                      onClick={() =>
                        update(
                          "DELETE",
                          el.id,
                          `http://localhost:1337/api/todos/${el.id}`
                        )
                      }
                    >
                      X
                    ...
                    <button
                      className=" pl-1 border-l border-white"
                  ...
                  <div className="flex gap-2 flex-wrap w-full">
                    ...
                    >
                      <textarea
                        ...
                        defaultValue={el?.attributes.description}

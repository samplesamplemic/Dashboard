...
export function UpcomingGames() {
  ...
  const fetching = useMemo(
    () =>
      axios.all(upcomingGamesUrl().map((el) => axios.get(el))).then(
        axios.spread(function (...res) {
          const key = "released";
          setData([
            ...new Map(
              res
                .map((el) => Object.values(el.data.results))
                .flat(1)
  ...
  const newArray = data.map((el) => [
    ...
    el.released,
    el.platforms,
    el.genres.map((subEl) => subEl.name),
    el.background_image,
  ]);
  ...
  const releaseDateFormat = (dateString) => {
    ...
    return (
      <p className="w-fit text-white px-1 flex gap-0.5 items-center">
        <span className="font-bold">{date.getDate()}</span>
        <span>|</span>
        <span className="text-ten">
  ...
  function gamePlatformID(platform) {
    return platform.map((subItem) =>
      subItem.platform.id === 1 || !subItem.platform.id === 186 ? (
        <span
          className="flex justify-center items-center w-3 h-3 bg-white rounded-full mx-px"
          key={subItem.platform.id}
        >
          <Icon className="text-gray-new w-2.5" path={mdiMicrosoftXbox} />
        </span>
      ) : subItem.platform.id === 187 || !subItem.platform.id === 18 ? (
        <span
          className="flex justify-center items-center w-3 h-3 bg-white rounded-full mr-px"
          key={subItem.platform.id}
        >
      ...
      <div className="flex flex-col items-center">
        ...
        <Slider className="slider big:!w-[96%]" {...settings}>
          {newArray.map((item, index) => (
            <div key={index}>
              <div
                className="rounded-tr-3xl rounded-bl-3xl rounded-br-lg rounded-tl-lg border border-white mt-2.5 mb-2.5 h-130px"
                style={{
              ...
              >
                <div className="bg-gradient-to-t from-black to-gradient-end flex flex-col justify-between w-full h-full rounded-tr-2xl rounded-br-lg rounded-tl-lg rounded-bl-3xl">
                  <div className="w-full">
                    <h4 className="px-1 text-white text-nine m-1 italic bg-neutral-400/50 rounded-lg w-fit text-center text-shadow-title">
                      {item[0]}
                    </h4>
                  </div>
